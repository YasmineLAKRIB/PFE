...
include('traitement/filtre.php');
  ...
  $tsql="select count(*) as c from [usthb90000L].[dbo].[CLASSEMENTL] where ANET=$annee and DESIGNATION='$spec'";
  ...
  foreach($results as $r){
   ...
   $tsql="select * from [usthb90000L].[dbo].[CLASSEMENTL] where ANET=$annee and DESIGNATION='$spec' order by Rang";
   $getresults2 = $conn->prepare($tsql);
   $getresults2->execute([$_POST['specialite']]);
   $results = $getresults2->fetchAll(PDO::FETCH_BOTH);
...
<head>
  <title>USTHB - Generation classement licence</title>
  ...
  <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active"><a href="index.html">Licence</a></li>
