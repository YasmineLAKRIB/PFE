...
$tsql="select top 1 fac from faculte0000 where faccode=(select top 1 faccode from filiere0000 where ocode='$ocode')";
    ...
    if(!empty($mat)){
    $tsql="select TOP 1 * from [usthb90000L].[dbo].[CLASSEMENTL] where mat='$mat' and ocode='$ocode' and ANET = $annee order by RANG";
                $getresults2 = $conn->prepare($tsql); 
            $getresults2->execute();
            $results = $getresults2->fetchAll(PDO::FETCH_BOTH);
            foreach($results as $r){
            ...
            $tsql="select * from [usthb90000L].[dbo].[CLASSEMENTL] where ocode='$ocode' and ANET = $annee order by RANG";
            $getresults2 = $conn->prepare($tsql); 
        $getresults2->execute();
        $results = $getresults2->fetchAll(PDO::FETCH_BOTH);
        foreach($results as $r){
...
foreach($results as $r){
    ...
    $pdf->SetLeftMargin(15);
    $pdf->SetFont('Times','',12);
    $pdf->Write($h, utf8_decode("            A obtenu son diplôme de Licence à l'année "));
